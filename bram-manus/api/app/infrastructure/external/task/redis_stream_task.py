...
class RedisStreamTask(Task):
    ...
    def __init__(self, task_runner: TaskRunner) -> None:
        ...
        input_stream_task = f"task:input:{self._id}"
        output_stream_task = f"task:output:{self._id}"

        self._input_stream = RedisStreamMessageQueue(input_stream_task)
        self._output_stream = RedisStreamMessageQueue(output_stream_task)
    ...
    def _on_task_done(self) -> None:
        """任务结束时的回调函数"""
        # 1.检测task_runner是否存在，若存在则调用task_runner的回调函数
