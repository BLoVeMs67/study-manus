...
class OpenAILLM(LLM):
    ...
    async def invoke(
        ...
        """使用异步OpenAI客户端发起快响应（该步骤可以切换成流式响应）"""
        try:
            # 1.检测是否传递了工具列表
            if tools:
                ...
                response = await self._client.chat.completions.create(
                    ...
                    parallel_tool_calls=False,  # 关闭并行工具调用(ds里没有这个参数)
                    timeout=self._timeout,
                )
            else:
                # 2.未传递工具则删除tools和tool_choice
                logger.info(f"调用OpenAI客户端向LLM发起请求，未携带工具信息：{self._model_name}")
                response = await self._client.chat.completions.create(
                    model=self._model_name,
                    temperature=self._temperature,
                    max_tokens=self._max_tokens,
