...
class DockerSandbox(Sandbox):
    ...
    async def destory(self) -> bool:
        ...
        try:
            ...
            logger.error(f"销毁当前DockerSandbox[{self._container_name}]失败：{str(e)}")
            return False
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not tool_result.success:
                    logger.warning(f"Supervisor进程检测失败：{tool_result.message}")
    ...
    async def upload_file(
        ...
        """上传文件至沙箱指定位置"""
        # 预配置上传数据
        files = {"file": (filepath or "upload", file_data, "application/octet-stream")}
