...
class BrowserTool(BaseTool):
    ...
    @tool(
        ...
        description="重启浏览器并访问对应的URL，当需要重置浏览器时使用",
        parameters={
            "url": {
                "type": "string",
                "description": "要访问的完整url，必须包含协议前缀（例如https://）"
    ...
    async def browser_click(
        ...
        """传递页面元素索引或者页面xy坐标点击对应的元素"""
        return await self.browser.click(index, coordinate_x, coordinate_y)

    @tool(
        name="browser_input",
        description="覆盖浏览器当前页面可编辑区域的文本（input/textarea输入框），在需要填充输入时使用",
        parameters={
            "text": {
                "type": "string",
    ...
    @tool(
        ...
        description="在当前浏览器页面模拟按键，当需要指定的键盘操作时使用",
        parameters={
            "key": {
                "type": "string",
                "description": "要模拟的按键名称（例如：Enter、Tab），支持组合键（例如：ctrl+C）"
            },
        },
        required=["key"]
    )
